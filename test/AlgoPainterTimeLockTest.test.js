...
const MineNftTimeLock = artifacts.require('MineNftTimeLock');
...
contract('MineNftToken', accounts => {
  ...
  it('should fail to do a emergency withdrawal before the specified time', async () => {
    ...
    try {
      ...
      expect(e.reason).to.be.equal('MineNftAccessControl: INVALID_ROLE');
    }
  ...
  it('should schedule several payments using schedulePayments with cliff/vesting', async () => {
    const now = new Date();
    now.setSeconds(now.getSeconds() + 20);
