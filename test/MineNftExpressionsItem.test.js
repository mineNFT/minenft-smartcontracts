...
contract.only('MineNftExpressionsItem', accounts => {
  let instance = null;

  it('should deploy the contracts', async () => {
    instance = await MineNftExpressionsItem.new(accounts[9], accounts[8]);
  });
  ...
  it('should fail to update a token URI based on an invalid validator', async () => {
    ...
    try {
      ...
      expect(e.reason).to.be.equal('MineNftExpressionsItem:INVALID_VALIDATOR', 'fail to check failure');
    }
  });
  ...
  it('should fail to update a token URI based on an invalid signature', async () => {
    ...
    try {
      ...
      expect(e.reason).to.be.equal('MineNftExpressionsItem:INVALID_SIGNATURE', 'fail to check failure');
    }
  });
  ...
  it('should fail to update a token URI based on an invalid sender', async () => {
    ...
    try {
      ...
      expect(e.reason).to.be.equal('MineNftExpressionsItem:INVALID_SENDER', 'fail to check failure');
    }
  });
});
